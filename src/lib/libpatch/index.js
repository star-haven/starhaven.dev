...
(function (_this) {
    ...
    const ERR_PATCH_CHECKSUM =
        "Patch checksum mismatch - patch file may be corrupt";
    const ERR_SOURCE_CHECKSUM =
        "Source checksum mismatch - patch is not meant for this ROM";
    ...
    function crc32(arr, offs, size) {
        ...
        for (var i = 0; i < size; i++) {
            crc =
                (CRC32Table[(crc & 0xff) ^ arr[offs + i]] ^ (crc >>> 8)) >>> 0;
        }
    ...
    function applyPatch(sourceData, patchData, ignoreChecksums) {
        ...
        for (var i in formats) {
            ...
            if (strtest(header, fmt.sig)) {
                ...
                console.log(
                    "libpatch: test applying " + fmt.name + " patch...",
                );
                timeStart = _this.performance.now();
                targetData = fmt.applyPatch(
                    sourceData,
                    patchData,
                    ignoreChecksums,
                );
                ...
                console.log(
                    "libpatch: took " + (timeEnd - timeStart).toFixed(3) + "ms",
                );
                return targetData;
            }
        }
    ...
    applyPatchAsync.WORKER_URL = (function () {
        ...
        var WORKER_SOURCE = [
            `importScripts(${JSON.stringify(import.meta.url)});`,
            "",
    ...
    function applyPatchUPS(sourceData, patchData, ignoreChecksums) {
        ...
        if (!ignoreChecksums) {
            ...
            if (
                patchChecksum !=
                crc32(patchStream.u8, 0, patchData.byteLength - 4)
            ) {
        ...
        if (
            !ignoreChecksums &&
            targetChecksum != crc32(targetU8, 0, targetU8.byteLength)
        ) {
    ...
    function applyPatchBPS(sourceData, patchData, ignoreChecksums) {
        ...
        var sourceSize = 0,
            ...
            metadataSize = 0;
        var targetOffs = 0,
            sourceRelativeOffs = 0,
            targetRelativeOffs = 0;
        ...
        for (var mode = 0; mode <= 5; mode++) {
            for (var addSize = 1; addSize <= 4; addSize++) {
                for (var copySize = 4; copySize <= 6; copySize++) {
                    table.push([
                        { inst: VCD_ADD, size: addSize, mode: 0 },
    ...
    VCDCache.prototype.decodeAddress = function (copyAddrStream, mode, here) {
        ...
        } else if ((m = mode - 2) >= 0 && m < this.nearSize) {
            // near cache
    ...
    function vcdPrecalculateTargetSize(patchStream) {
        ...
        while (!patchStream.isEOF()) {
            ...
            patchStream.skip(
                winHeader.dataLength +
                    winHeader.copysLength +
                    winHeader.instrsLength,
            );
        }
    ...
    function applyPatchVCD(sourceData, patchData, ignoreChecksums) {
        ...
        while (!patchStream.isEOF()) {
            ...
            if (winHeader.deltaIndicator & VCD_INSTCOMP) {
                ...
                instructionStream = new VCDStream(
                    patchData,
                    dataStream.offset + winHeader.dataLength,
                );
            }

            if (winHeader.deltaIndicator & VCD_ADDRCOMP) {
            } else {
                copyAddrStream = new VCDStream(
                    patchData,
                    instructionStream.offset + winHeader.instrsLength,
                );
            }
            ...
            while (instructionStream.offset < instructionStreamEndOffs) {
                    ...
                    switch (op) {
                        case VCD_ADD:
                            dataStream.readBytes(
                                targetU8,
                                targetWindowPosition + targetWindowOffs,
                                length,
                            );
                        ...
                        case VCD_COPY:
                            ...
                            if (addr >= winHeader.sourceSegmentLength) {
                                absAddr =
                                    ...
                                    (addr - winHeader.sourceSegmentLength);
                                copySourceU8 = targetU8;
                            } else {
                                absAddr =
                                    winHeader.sourceSegmentPosition + addr;
                                if (winHeader.indicator & VCD_SOURCE) {
                                    copySourceU8 = sourceU8;
                                }
                            }
                ...
                progress.update(
                    (targetWindowPosition + targetWindowOffs) / targetSize,
                );
            }

            if (winHeader.haveChecksum && !ignoreChecksums) {
                var testAdler32 = adler32(
                    targetU8,
                    targetWindowPosition,
                    winHeader.targetWindowLength,
                );
...
export const applyPatch = libpatch.applyPatch;
export const applyPatchAsync = libpatch.applyPatchAsync;
export const performance = libpatch.performance;
